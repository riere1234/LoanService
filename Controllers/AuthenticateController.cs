...
namespace LoanService.Controllers
{
    public class AuthenticateController : ControllerBase
    {
        ...
        private async Task<LoginModel> AuthenticateUser(LoginModel login)
        {
            ...
            string callID = _config.GetSection("callID").Value;
            string callkey = _config.GetSection("callkey").Value;
            if (login.UserName == callID && login.Password ==callkey)
            {
                user = new LoginModel { UserName = callID };
            }
            return user;
        }
        #endregion
